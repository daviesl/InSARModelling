...
def mogiDEM__(x0_,y0_,z0_,dV_,x,y,dem):
	"""evaluate a single Mogi peak over a 2D (2 by N) numpy array of evalpts, where coeffs = (x0,y0,z0,dV)"""
	#x0,y0,z0,dV = coeffs
	dx = x - x0_
	dy = y - y0_
...
with basic_model:
	...
	x0 = pm.Uniform('x0',ldE-hstd,ldE+hstd)
	y0 = pm.Uniform('y0',ldN-hstd,ldN+hstd)
	z0 = pm.Uniform('z0',ld[2]-vstd,ld[2]+vstd)
	dV = pm.Uniform('dV',0,5e6)
	...
	def logp(x0,y0,z0,dV):
		return (synthetic1(x0,y0,z0,dV)+synthetic2(x0,y0,z0,dV)+synthetic3(x0,y0,z0,dV)) * (-0.5) # for DensityDist: * (-0.5)

	#llk1 = pm.HalfNormal('llk1',sd=1,observed=synthetic1(x0,y0,z0,dV))
	#llk2 = pm.HalfNormal('llk2',sd=1,observed=synthetic2(x0,y0,z0,dV))
	#llk3 = pm.HalfNormal('llk3',sd=1,observed=synthetic3(x0,y0,z0,dV))

	like = pm.DensityDist('like',logp,observed={'x0':x0,'y0':y0,'z0':z0,'dV':dV})
	#like = pm.HalfNormal('like',sd=1,observed=logp(x0,y0,z0,dV))
