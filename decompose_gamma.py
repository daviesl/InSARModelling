...
def parse():
    ...
    parser.add_argument('-dp', action='store', dest = "dem_par_name", default=None, help="DEM parameter file as out[ut by GAMMA", type=str)
    parser.add_argument('-th', action='store', dest = "theta_name", default=None, help="Elevation angle (theta) file with same dimensions as geocoded unwrapped file", type=str)
    parser.add_argument('-ph', action='store', dest = "phi_name", default=None, help="Azimuth (phi) file with same dimensions as geocoded unwrapped file", type=str)
    parser.add_argument('-cor', action='store', dest = "cor_name", default=None, help="Coherence file with same dimensions as geocoded unwrapped file", type=str)
    parser.add_argument('-ct', action='store', required=True, dest='corthresh', help='Coherence threshold for masking regions of the interferogram with low coherence',type=float)
    ...
    #########Check input options
    if np.sum(np.array([inps.useaz,inps.usedefgeom,(inps.theta_name is not None or inps.phi_name is not None)]))>1:
...
indata = gamma_reader.gamma_reader(inps.dem_par_name, inps.I_name, inps.theta_name, inps.phi_name, inps.cor_name)
...
if inps.plot:
    ...
    newz = griddata((sampler.xi,sampler.yi),sampler.zi,(x[None,:],y[:,None]),method='linear') # was method='cubic'
...
if inps.covar:
    (sigma,lam) = covaraps.aps_param_proj(indata.phs, inps.nfrac, indata.x, indata.y, inps.dscale, inps.plot)
    ...
    #        distmn[m,n] = dist
    # vectorised solution for speed
    
    #dx = np.tile(indata.x[sampler.xi-1],npts) - np.repeat(indata.x[sampler.xi-1],npts)
    #dy = np.tile(indata.y[sampler.yi-1],npts) - np.repeat(indata.y[sampler.yi-1],npts)
    #dist = inps.dscale*np.sqrt(dx*dx+dy*dy)
    geod = pyproj.Geod(ellps='WGS84')
    _az12, _az21, dist = geod.inv(np.tile(indata.x[sampler.xi-1],npts),np.tile(indata.y[sampler.yi-1],npts),np.repeat(indata.x[sampler.xi-1],npts),np.repeat(indata.y[sampler.yi-1],npts)) #,radians=True)
    dist = dist.reshape((npts,npts))
    cvar = covaraps.model_fn(dist,sigma,lam)
    cvar = sigma - cvar
	
    #Error component due to resampling
    #for m in xrange(npts):
    #     cvar[m,m] += sampler.zerr[m]*sampler.zerr[m]
    cvar += np.diag(np.array(sampler.zerr[:npts])**2)
    ...
    if inps.plot:
        #cvar_m = np.ma.masked_where(cvar==0, cvar)
	plt.figure('Covariance')
        cmap = plt.cm.gist_rainbow_r
        cmap.set_under(color='white')
	plt.imshow(cvar,interpolation='none',cmap=cmap,vmin=0.0000001)
	plt.colorbar(aspect = 8 , shrink = 0.6)
	plt.title('Covariance (cm^2)')
	plt.figure('Covar one axis')
	plt.plot(cvar[:,0])
	plt.title('covar one axis')
        # plot distance vs error against sigma^2 exp(-d_ij / lambda)
        plt.figure('Distance vs error')
        plt.scatter(dist.flatten()/inps.dscale,cvar.flatten())
        plt.title('Distance vs Covariance')
