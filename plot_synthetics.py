...
plot_varres(ax1,par_D,T025D[:,3],T025D[:,4],T025D_los,7.5)
ax2 = fig.add_subplot(334,sharex=ax1)
plot_varres(ax2,par_D,T025D[:,3],T025D[:,4],T025D[:,5],7.5,T025Dmask)
ax2b = fig.add_subplot(337,sharex=ax1)
#diff1 = np.absolute((T025D_los - T025D[:,5])/T025D[:,6])
diff1 = np.absolute((T025D_los - T025D[:,5]))
plot_varres(ax2b,par_D,T025D[:,3],T025D[:,4],diff1,7.5,None,0,diff1.max(),'jet')

ax3 = fig.add_subplot(332,sharey=ax1)
plot_varres(ax3,par_A,T131A[:,3],T131A[:,4],T131A_los,7.5)
ax4 = fig.add_subplot(335,sharex=ax3,sharey=ax2)
plot_varres(ax4,par_A,T131A[:,3],T131A[:,4],T131A[:,5],7.5,T131Amask)
ax4b = fig.add_subplot(338,sharex=ax3,sharey=ax2b)
#diff2 = np.absolute((T131A_los - T131A[:,5])/T131A[:,6])
diff2 = np.absolute((T131A_los - T131A[:,5]))
plot_varres(ax4b,par_D,T131A[:,3],T131A[:,4],diff2,7.5,None,0,diff2.max(),'jet')

ax5 = fig.add_subplot(333,sharey=ax1)
plot_varres(ax5,par_A2,T130A[:,3],T130A[:,4],T130A_los,7.5)
ax6 = fig.add_subplot(336,sharex=ax5,sharey=ax2)
plot_varres(ax6,par_A2,T130A[:,3],T130A[:,4],T130A[:,5],7.5,T130Amask)
ax6b = fig.add_subplot(339,sharex=ax5,sharey=ax2b)
#diff3 = np.absolute((T130A_los - T130A[:,5])/T130A[:,6])
diff3 = np.absolute((T130A_los - T130A[:,5]))
plot_varres(ax6b,par_D,T130A[:,3],T130A[:,4],diff3,7.5,None,0,diff3.max(),'jet')
